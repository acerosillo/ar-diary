...
// envVar is APP_PASSCODE for the main journal (read/write) or
// SHARED_PASSCODE for the read-only page — two separate secrets, since
// handing out the read-only one must never also grant write access to
// the whole journal.
function checkPasscode(req, envVar) {
  // .trim() on both sides: a passcode pasted into Netlify's env var field
  // (or typed on a phone) all too easily picks up a stray leading/trailing
  // space or newline, which would otherwise fail silently as "incorrect".
  const expected = (process.env[envVar] || '').trim();
  if (!expected) return { ok: false, status: 500, error: `Not set up yet — ${envVar} is not configured.` };
  const given = ((req.headers['x-passcode'] || req.query.passcode || '') + '').trim();
...
function createApiApp(db) {
  ...
  // Everything except /shared is the main journal — full read/write, so it
  // needs its own passcode, checked on every request.
  app.use((req, res, next) => {
    if (req.path === '/shared') return next();
    const check = checkPasscode(req, 'APP_PASSCODE');
  ...
  app.get('/shared', async (req, res) => {
    const check = checkPasscode(req, 'SHARED_PASSCODE');
    if (!check.ok) return res.status(check.status).json({ error: check.error });
    try {
